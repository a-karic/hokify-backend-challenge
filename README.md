...
* Today (02.08.2023), I added 2 new components: JsonInput and JsonOutput, and improved the ConfirmDialog component. With these changes, there is now an auto-check to see if the entered JSON is valid before submitting the changes. This is done to improve the UI/UX of this small app so that the user knows when the entered JSON is valid or not. The JsonOutput component is there to format the JSON when displaying it on the UI.
